...
import com.ceos.beatbuddy.domain.member.dto.VerifyPhoneNumberDTO;
...
public interface BusinessMemberApiDocs {
    @Operation(
            summary = "비즈니스 사용자 전화번호 확인, 인증번호 요청",
    ...
    ResponseEntity<ResponseDTO<VerificationCodeResponseDTO>> verifyPhoneNumber(@RequestBody VerifyPhoneNumberDTO dto);

    @Operation(
            summary = "비즈니스 사용자 본인 확인",
            description = """
            전화번호, 주민등록번호 앞 7자리, 실명을 입력하여 본인 인증을 진행합니다.  
            현재는 테스트 단계로 인증번호 없이 통과됩니다.
        """
    )
    @ApiResponses(value = {
            @ApiResponse(
    ...
    ResponseEntity<ResponseDTO<BusinessMemberResponseDTO>> verifyBusinessMember(@RequestBody BusinessMemberDTO dto);
}
