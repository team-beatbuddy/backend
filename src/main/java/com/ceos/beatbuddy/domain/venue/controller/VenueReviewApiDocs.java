...
public interface VenueReviewApiDocs {
    ...
    @Operation(
            summary = "베뉴 리뷰 수정하기",
            description = "베뉴 리뷰를 수정합니다. 리뷰 내용은 최대 400자까지 입력할 수 있으며, 이미지가 5개까지 첨부 가능합니다. " +
                    "이미지는 필수 사항이 아니며, 첨부하지 않아도 리뷰 수정이 가능합니다. " +
                    "multipart/form-data 형식으로 요청해야 하며, 파일 1개의 최대 크기는 10MB입니다. " +
                    "전체 요청 크기는 30MB를 초과할 수 없습니다."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "베뉴 리뷰 수정 성공",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = ResponseDTO.class),
                            examples = {@ExampleObject(value = """
                                    {
                                      "status": 200,
                                      "code": "SUCCESS_UPDATE_VENUE_REVIEW",
                                      "message": "베뉴 리뷰를 수정했습니다.",
                                      "data": {
                                        "venueReviewId": 6,
                                        "content": "수정된 리뷰 내용",
                                        "nickname": "길동hong",
            ...
            @ApiResponse(responseCode = "500", description = "S3에 이미지 등록/삭제 실패했을 경우",
                    content = @Content(
                            mediaType = "application/json",
                            examples = {
                                    @ExampleObject(
                                            name = "s3에 이미지 등록을 실패했을 경우", value = SwaggerExamples.IMAGE_UPLOAD_FAILED),
                                    @ExampleObject(
                                            name = "s3에 이미지 삭제를 실패했을 경우", value = SwaggerExamples.IMAGE_DELETE_FAILED)
                            }
                    )
            )
    })
